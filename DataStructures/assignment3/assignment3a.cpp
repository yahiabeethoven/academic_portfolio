...
unsigned long HashTable::hashCode(const string key)	//polynomial hashfunction
{
	int length = key.length();
	int order = key.length() - 1;								//store size of string

		if (length == 0) {										//if string is empty then return 0
			return 0;
		}

		unsigned long code = 0;									//initialize hash code to 0

		for (int i = 0; i < length; i++) {						//iterate over characters of key
			code += (unsigned long)(key[i]*(pow(41, order)));		//chose this this since it was proven to reduce collisions in an experiment, accumulate to the code the char times 37 to the power n-1
			order--;
		}
