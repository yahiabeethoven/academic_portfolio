...
class HashNode
{
	...
	public:
		...
		{
			this->key = key;
			this->frequency = frequency;								//attributes of each node
		}
...
HashTable::HashTable(int capacity)
{
	...
	this->collisions = 0;			
	this->wordRepeated = 0;
	this->totalWords = 0;								//initialize all values to 0
}

unsigned int HashTable::hashCode(const string key)	//cycle-shift hashfunction
{
	int length = key.length();
		...
		unsigned int code = 0;									//initialize hash code to 1
